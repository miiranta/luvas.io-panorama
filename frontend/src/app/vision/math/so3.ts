...
import { jacobiEigen } from './decomposition';
...
export function axisAngleFromRotation(m: Mat3): [number, number, number] {
    ...
    if (Math.abs(s) < 1e-9) {
        const rx = Math.sqrt(Math.max(0, (m[0] + 1) / 2));
        const ry = Math.sqrt(Math.max(0, (m[4] + 1) / 2));
        const rz = Math.sqrt(Math.max(0, (m[8] + 1) / 2));
        return [rx * theta, ry * theta, rz * theta];
    }
    return [((m[7] - m[5]) / s) * theta, ((m[2] - m[6]) / s) * theta, ((m[3] - m[1]) / s) * theta];
}
