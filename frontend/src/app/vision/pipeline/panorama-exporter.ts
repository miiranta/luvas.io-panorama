...
export class PanoramaExporter {
    ...
    async render(
        ...
        for (const frame of frames) {
            ...
            if (source) sizes.set(frame.id, { width: source.width, height: source.height });
        }
        const usable = frames.filter((frame) => sizes.has(frame.id));
