...
import { MosaicFactory, MosaicSurface, MosaicView } from '../compositing/mosaic-surface';
import { WarpTile } from '../compositing/warp-tile';
...
export class MosaicBackend {
    ...
    factory(enabled: boolean): MosaicFactory {
        ...
        return (width: number, height: number, bands: number, view?: MosaicView) => {
            ...
            return new Mosaic(width, height, bands, view);
        };
    }
    ...
    private calibrate(): Verdict {
        ...
        const expected = compose(new Mosaic(CHECK_WIDTH, CHECK_HEIGHT, CHECK_BANDS), checkTiles);
        ...
        const expectedWrap = compose(
            new Mosaic(WRAP_WIDTH, WRAP_HEIGHT, CHECK_BANDS, view),
            wrapTiles,
        );
        ...
        const cpuMs = time(() => new Mosaic(TIMING_WIDTH, TIMING_HEIGHT, CHECK_BANDS));
        const gpuMs = time(() =>
